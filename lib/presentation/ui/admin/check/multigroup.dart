...
class MultiGroupListPage extends StatefulWidget {
  final List<Member> members;

  MultiGroupListPage({this.members});
  @override
  _MultiGroupListPage createState() => _MultiGroupListPage(members);
}

//???OTMECHATY ODNOGO I TOGO ZHE V RAZNIH SEKCIJAH ILI ODIN RAZ????
class _MultiGroupListPage extends State<MultiGroupListPage> {
  List<Member> members;
  bool notCheckedToday = true;
  List<String> presentList = List();
  List<String> absentList = List();
  _MultiGroupListPage(this.members);

  @override
  void initState() {
    CheckService.isTodayListCheckedStream.listen((event) {
      var doc = event as DocumentSnapshot;
      var timestamp = doc.data()['multi_group'] as Timestamp;
    ...
    members = members
        .where((member) => member.section.contains('multi_group_section'))
        .toList();
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () {
          ...
          CheckService.checkPeople(presentList, absentList)
              .then((value) => CheckService.fixMultiGroupTodayCheckList());
        },
      ),
    );
  }
