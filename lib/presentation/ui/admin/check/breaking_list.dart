...
class BreakingListPage extends StatefulWidget {
  final List<Member> members;

  BreakingListPage(this.members);
  @override
  _BreakingListPage createState() => _BreakingListPage(members);
}

class _BreakingListPage extends State<BreakingListPage> {
  List<Member> members;
  bool notCheckedToday = true;
  List<String> presentList = List();
  List<String> absentList = List();
  _BreakingListPage(this.members);
  @override
  void initState() {
    CheckService.isTodayListCheckedStream.listen((event) {
      var doc = event as DocumentSnapshot;
      var timestamp = doc.data()['breaking'] as Timestamp;

      if (timestamp != null) {
        var date = timestamp.toDate();
        setState(() {
          notCheckedToday = (DateTime.now().difference(date).inSeconds > 6);
        });
      }
    });
    members = members
        .where((member) => member.section.contains('breaking_section'))
        .toList();
    super.initState();
  }
    ...
    return Scaffold(
            ...
            child: Container(
              ...
              child: Padding(
                  ...
                  child: Container(
                      ...
                      child:
                          // StreamBuilder(
                          //     stream: CheckService.isTodayListCheckedStream,
                          //     builder: (_, snapshot) {
                          //       if (snapshot.hasData &&
      ...
      floatingActionButton: FloatingActionButton(
        ...
        onPressed: () async {
          ...
          await CheckService.checkPeople(presentList, absentList);
          await CheckService.fixBreakingTodayCheckList();
        },
      ),
    );
  }
